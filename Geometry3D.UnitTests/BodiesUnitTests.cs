...
namespace Geometry3DTests
{
    ...
    public class BodiesUnitTests
    {
        ...
        private static void AssertCompoundBodyWithSinglePart_Contains_IsEquivalentToItsPartContains(Body part)
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                var p = new Vector3(
                    ...
                    random.NextDouble() * 20 - 10,
                    random.NextDouble() * 20 - 10);
                Assert.That(compound.ContainsPoint(p), Is.EqualTo(part.ContainsPoint(p)));
                Assert.That(compound2.ContainsPoint(p), Is.EqualTo(part.ContainsPoint(p)));
            }
        }
        ...
        {
            Assert.That(body.ContainsPoint(new Vector3(x, y, z)), Is.EqualTo(expected));
        }
    }
}
